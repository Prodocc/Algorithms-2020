...
abstract class AbstractBinarySearchTreeTest {
    ...
    protected fun doRemoveTest() {
        ...
        val controlSet = mutableSetOf<Int>()
        controlSet.addAll(arrayOf(1, 2, 3, 4, 5, 6, 7))
        val toRemove = 1
    ...
    protected fun doIteratorTest() {
        ...
        for (iteration in 1..100) {
            ...
            assertFailsWith<java.lang.IllegalStateException>("Something was supposedly returned after the elements ended") {
                binaryIter.next()
            }
            println("All clear!")
        }
        val controlSet = TreeSet<Int>()
        controlSet.addAll(arrayOf(72, 4, 1, 0, 4, 9, 99, 2))
        println("Control set: $controlSet")
        val binarySet = create()
        assertFalse(
            binarySet.iterator().hasNext(),
